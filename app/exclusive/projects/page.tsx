  ...
  return (
    <section className="min-h-screen px-6 py-16 lg:py-24">
      ...
      <p className="text-[var(--text-secondary)] mb-12">
        Эксклюзивный контент для держателей токенов
      </p>
      ...
      <div className="p-8 bg-[var(--bg-card)]/60 backdrop-blur-xl border border-[var(--border)]/60 rounded-lg">
        {uploadedFiles.length > 0 ? (
                ...
                <div className="flex items-center justify-between gap-4 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="audio-progress cursor-pointer w-24"
                    />
                  </div>
                </div>
                  ...
                  {uploadedFiles.filter(isTrack).map((f) =>
                    fileAccess[f.path] ? (
                      <AudioPlayer
                        ...
                        onToggleFavorite={() => toggleFavorite(f.path)}
                      />
                    ) : (
                      <div
                        key={f.id}
                        className="flex items-center gap-4 p-4 bg-[var(--bg-secondary)]/60 backdrop-blur-md rounded-lg animate-pulse"
                      >
                  ...
                  {uploadedFiles.filter((f) => !isTrack(f)).map((f) => (
                    <div key={f.id} className="relative group/card">
                      ...
                      <div className="absolute top-2 right-2 z-10 opacity-100 sm:opacity-0 sm:group-hover/card:opacity-100 transition-opacity">
                        <FavoriteButton
                          isFavorite={favorites.includes(f.path)}
                          onToggle={() => toggleFavorite(f.path)}
                        />
                      </div>
                    </div>
                  ))}
          ...
          <p className="text-[var(--text-secondary)]">
            Контент появится здесь. Админ загружает файлы в панели управления.
          </p>
        )}
      </div>
    </section>
