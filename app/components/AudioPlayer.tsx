...
function FavoriteButtonInline({ isFavorite, onToggle, className = '' }: { isFavorite: boolean; onToggle: () => void; className?: string }) {
    ...
    >
      ...
      <span className={`relative z-10 block transition-transform duration-200 ${burst ? 'scale-125' : ''}`}>
        {isFavorite ? (
          ...
          <svg className="w-5 h-5 text-[var(--text-muted)] hover:text-rose-400 fill-none stroke-current stroke-2" viewBox="0 0 24 24">
...
export default function AudioPlayer({ id, path, token, address, name, coverPath, playingId, onPlay, volume, isMuted, isFavorite, onToggleFavorite }: AudioPlayerProps) {
